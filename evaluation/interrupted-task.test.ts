...
test('interrupted-task scenario recalls task state and session summary for resumption', async () => {
  const scenario = await loadScenarioDocument()
  const tempDirectory = await mkdtemp(join(tmpdir(), 'info7375-interrupted-task-'))
  ...
  try {
    ...
    assert.equal(scenario.id, 'interrupted-task')
    ...
    assert.match(memoryBundle.text, /Phase 1 runtime scaffolding is committed/)
    assert.match(memoryBundle.text, /runtime-first scaffold, passing tests/)
  } finally {
...
async function loadScenarioDocument(): Promise<ScenarioDocument> {
  const scenarioUrl = new URL('./scenarios/interrupted-task.json', import.meta.url)
  const content = await readFile(scenarioUrl, 'utf8')
  const parsed: unknown = JSON.parse(content)

  if (!isScenarioDocument(parsed)) {
    throw new Error('interrupted-task scenario has an invalid shape')
  }
